...
def compute_metrics(eval_pred):
    ...
    # BART 모델은 디코딩된 토큰을 제공하기 때문에 디코딩하여 문자열로 변환
    generated_sentences = [pred["generated_text"] for pred in predictions]

    # BLEU 점수 계산
    bleu_score = metric_bleu.compute(predictions=generated_sentences, references=labels)

    # 정확도 계산을 위해 생성된 문장과 레이블 비교
    # 여기서는 완전히 일치하는 문장이 있는지 여부를 확인하여 정확도를 계산
    accuracy = accuracy_score(labels, generated_sentences)

    scores = {"bleu": bleu_score["score"], "accuracy": accuracy}
...
training_args = TrainingArguments(
    ...
    num_train_epochs=100,  # 학습 에포크 설정
